...
describe("concat", () => {
  it("should concatenate 'Mohamed ' and 'Hassan'", () => {
    const result = concat("Mohamed ", "Hassan");
    expect(result).to.equal("Mohamed Hassan");
  });

  it("should throw if one of the strings is empty", () => {
    expect(() => {
      concat("Mohamed", "");
    }).to.throw("the two strings must not be empty");
  });
});
